...
:::info
Did you know that in the new Riverscape Network verson of [BRAT 5.0](https://tools.riverscapes.net/brat) there is a new intermediate output `Beaver Dam Setting` layer that has these estimates of classic, steep and floodplain *beaver dam building opportunties*. 

Explore and download [these models in the Data Exchange](https://data.riverscapes.net/s?type=Project&projectTypeId=riverscapes_brat&view=table) for all of CONUS!
:::
---

## Schematics of Inundation Contrasted

In each of the 36 sites below, schematics of inundation within the valley bottom (white) are shown as pairs with the same site without beaver dam activity on top, and with beaver dam activity on the bottom. This is the primary mapping result of RIM (i.e. digitizing inundation extent). In each figure, the first column are sites within **classic** settings (i.e. < 6% slope and damming main channel), **steep** settings (>6% slope) and **floodplain** settings (dams primarily on floodplain and side channels). [citation](https://digitalcommons.usu.edu/etd/8226/).
